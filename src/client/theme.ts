...
/**
 * Runtime stylesheet. The `cmp-*` class names mirror the small daisyUI 5
 * primitive vocabulary used by the surface, while every selector remains
 * Companion-scoped and Tailwind Preflight is deliberately absent. The build
 * instructions live in the README; shipping raw at-rules here would make a
 * browser request `/companion/tailwindcss` instead of treating this as CSS.
 */
...
#dsh-companion .companion-app { width: min(1180px, 100%); min-height: 100dvh; margin: 0 auto; display: grid; grid-template-columns: minmax(0, 1fr) 280px; }
...
#dsh-companion .companion-voice progress { width: 90px; accent-color: var(--color-primary); }
...
#dsh-companion .companion-side { border-left: 1px solid color-mix(in srgb, var(--color-base-content) 12%, transparent); padding: 26px 18px; background: color-mix(in srgb, var(--color-base-200) 44%, transparent); }
#dsh-companion .companion-side-card { border-radius: 24px; padding: 20px; background: color-mix(in srgb, var(--color-base-100) 82%, transparent); box-shadow: var(--cmp-shadow); }
...
#dsh-companion .companion-detail { position: fixed; inset: 0; z-index: 20; display: grid; place-items: center; background: rgb(20 15 28 / .42); padding: 20px; }
#dsh-companion .companion-detail-card { width: min(420px, 100%); border-radius: 28px; padding: 28px; background: var(--color-base-100); box-shadow: var(--cmp-shadow); }
...
#dsh-companion .companion-lightbox img { max-width: min(94vw, 1000px); max-height: 86vh; object-fit: contain; border-radius: 15px; }
...
@media (max-width: 820px) { #dsh-companion .companion-app { display: block; } #dsh-companion .companion-side { display: none; } #dsh-companion .companion-header { padding-inline: 14px; } #dsh-companion .companion-timeline { padding-inline: 12px; } #dsh-companion .companion-row { max-width: 92%; } }
