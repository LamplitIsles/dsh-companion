...
- Added the public token-meter and compaction event type dependencies at the pinned `0.1.1-rc.2` versions.
...
- Added one durable, non-expandable `整理记录` per successful compaction, using only the safe rounded `shadowedTokenCount` evidence and never rendering the private summary.
